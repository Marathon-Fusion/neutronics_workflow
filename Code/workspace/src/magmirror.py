...
reactor_power = 1e9 #Wth
major_r = 420 #cm, from paper design point

reactor_energy = reactor_power * 365.25 * 24 * 60 * 60 #reactor (thermal) energy in J in one year
circum = 2*np.pi*major_r
slice_proportion = cylinder_length/circum #how much of full tokamak this slice represents
...
n_per_year_file = os.path.join(results_dir, 'n_per_year_per_slice.txt')
with open(n_per_year_file, 'w') as output:
    output.write(str(n_per_year_per_slice))
