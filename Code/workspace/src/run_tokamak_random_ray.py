...
def make_random_ray_source(major_rad, minor_rad, name, source_cell, energy=14.1e6, centre=[0,0,0], plot = False):

    x0, y0, z0 = centre

    source_region = openmc.ZTorus(x0, y0, z0, major_rad, minor_rad)

    n_source = openmc.IndependentSource()
    n_source.constraints={'domains': [source_cell]}
    n_source.energy = openmc.stats.Discrete([energy], [1.0]) #14.1MeV neutrons only
    n_source.name = name

    print(f"Neutron random ray source created")
...
source = make_random_ray_source(major_rad = 385, minor_rad=10, name='torus_source', source_cell=border_cell)
print("Created neutron source...")
...
settings.random_ray = {'ray_source': source,
                       'distance_inactive': 10,
                       'distance_active': 300}
